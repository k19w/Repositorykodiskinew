...
def auto_install_repo():
    if not os.path.exists(os.path.join(CONFIG.ADDONS, CONFIG.REPOID)):
        ...
        if response:
            ...
            logging.log_notify("[COLOR {0}]שגיאת התקנת ריפו[/COLOR]".format(CONFIG.COLOR1),
                               "[COLOR {0}]קובץ addon.xml לא חוקי![/COLOR]".format(CONFIG.COLOR2))
            logging.log("[Auto Install Repo] לא ניתן לקרוא את הקובץ addon.xml.", level=xbmc.LOGERROR)
    elif not CONFIG.AUTOINSTALL == 'Yes':
        logging.log("[התקן אוטומטית רפו] לא מופעל", level=xbmc.LOGINFO)
    elif os.path.exists(os.path.join(CONFIG.ADDONS, CONFIG.REPOID)):
        logging.log("[התקן אוטומטית רפו] מאגר כבר מותקן")
...
def installed_build_check():
    ...
    if not CONFIG.EXTRACT == '100' and CONFIG.EXTERROR > 0:
        logging.log("[בילד בדיקת התקנה]  בניין חולץ  {0}/100  עם [ERRORS: {1}]".format(CONFIG.EXTRACT,
                                                                                                    CONFIG.EXTERROR),
                    level=xbmc.LOGINFO)
        yes = dialog.yesno(CONFIG.ADDONTITLE,
                           '[COLOR {0}]{2}[/COLOR] [COLOR {1}]לא הותקן כהלכה![/COLOR]'.format(CONFIG.COLOR1,
                                                                                                   CONFIG.COLOR2,
                                                                                                   CONFIG.BUILDNAME),
                           ('Installed: [COLOR {0}]{1}[/COLOR] / '
                            'Error Count: [COLOR {2}]{3}[/COLOR]').format(CONFIG.COLOR1, CONFIG.EXTRACT, CONFIG.COLOR1,
                                                                          CONFIG.EXTERROR),
                           'האם ברצונך לנסות שוב?[/COLOR]', nolabel='[B]לא תודה![/B]',
                           yeslabel='[B]נסה להתקין מחדש[/B]')
        ...
        if not CONFIG.SKIN == defaults and not CONFIG.BUILDNAME == "":
            ...
            if not response:
                ...
                dialog.ok(CONFIG.ADDONTITLE,
                          "[COLOR {0}]נראה כי הגדרות העור לא הוחלו על הבילד.".format(CONFIG.COLOR2),
                          "למרבה הצער שום תיקון גוי לא צורף בילד",
                          "יהיה עלייך להתקין מחדש את ה-בילד[/COLOR]")
            else:
                yes = dialog.yesno(CONFIG.ADDONTITLE,
                                       ...
                                       nolabel='[B]לא, בטל[/B]', yeslabel='[B]החל תקן[/B]')
                if yes:
        ...
        logging.log('[Build Installed Check] נראה כי ההתקנה הושלמה כהלכה', level=xbmc.LOGINFO)
        
    if CONFIG.get_setting('installed') == 'true':
        if CONFIG.get_setting('keeptrakt') == 'true':
            from resources.libs import traktit
            logging.log('[Build Installed Check] שחזור נתוני טרקט', level=xbmc.LOGINFO)
            traktit.trakt_it('restore', 'all')
        if CONFIG.get_setting('keepdebrid') == 'true':
            from resources.libs import debridit
            logging.log('[Build Installed Check] שחזור נתוני פסולת אמיתית', level=xbmc.LOGINFO)
            debridit.debrid_it('restore', 'all')
        if CONFIG.get_setting('keeplogin') == 'true':
            from resources.libs import loginit
            logging.log('[Build Installed Check] שחזור נתוני הכניסה', level=xbmc.LOGINFO)
...
def build_update_check():
    ...
    if not response:
        logging.log("[Build Check] לא אותר כתובת קובץ בילד: {0}".format(CONFIG.BUILDFILE), level=xbmc.LOGINFO)
    elif not CONFIG.BUILDNAME == '':
        if CONFIG.SKIN in ['skin.confluence', 'skin.estuary', 'skin.estouchy'] and not CONFIG.DEFAULTIGNORE == 'true':
            check.check_skin()

        logging.log("[Build Check]בילד מותקן בודק עדכונים", level=xbmc.LOGINFO)
...
def save_trakt():
    ...
    if next_save <= current_time:
        from resources.libs import traktit
        logging.log("[Trakt Data] שומר את כל הנתונים", level=xbmc.LOGINFO)
...
def auto_clean():
    ...
    if next_cleanup <= tools.get_date() or freq == 0:
        ...
        logging.log("[Auto Clean Up] הבא לנקות {0}".format(CONFIG.NEXTCLEANDATE),
                    level=xbmc.LOGINFO)
    if service:
        if CONFIG.AUTOCACHE == 'true':
            logging.log('[Auto Clean Up] מטמון: מופעל', level=xbmc.LOGINFO)
            clear.clear_cache(True)
        else:
            logging.log('[Auto Clean Up] מטמון: כבוי', level=xbmc.LOGINFO)
...
if CONFIG.get_setting('first_install') == 'true':
    logging.log("[First Run] מציג הגדרות שמירת נתונים", level=xbmc.LOGINFO)
...
if tools.open_url(CONFIG.BUILDFILE, check=True) and CONFIG.get_setting('installed') == 'false':
    logging.log("[Current Build Check] ביטול התקנת בילד", level=xbmc.LOGINFO)
    window.show_build_prompt()
else:
    logging.log("[Current Build Check] התקנת בילד: {0}".format(CONFIG.BUILDNAME), level=xbmc.LOGINFO)
...
if CONFIG.AUTOINSTALL == 'Yes':
    logging.log("[Auto Install Repo] התחיל", level=xbmc.LOGINFO)
    auto_install_repo()
else:
    logging.log("[Auto Install Repo] לא מופעל", level=xbmc.LOGINFO)

# REINSTALL ELIGIBLE BINARIES
binarytxt = os.path.join(CONFIG.USERDATA, 'build_binaries.txt')
if os.path.exists(binarytxt):
    logging.log("[Binary Detection] התקנה מחדש של תוספים בינאריים זכאים", level=xbmc.LOGINFO)
    from resources.libs import restore
    restore.restore('binaries')
else:
    logging.log("[Binary Detection] תוספים בינאריים זכאים להתקנה מחדש", level=xbmc.LOGINFO)
    
# AUTO UPDATE WIZARD
if CONFIG.AUTOUPDATE == 'Yes':
    logging.log("[Auto Update Wizard] התחיל", level=xbmc.LOGINFO)
    update.wizard_update()
else:
    logging.log("[Auto Update Wizard] לא מופעל", level=xbmc.LOGINFO)

# SHOW NOTIFICATIONS
if CONFIG.ENABLE_NOTIFICATION == 'Yes':
    show_notification()
else:
    logging.log('[Notifications] לא מופעל', level=xbmc.LOGINFO)

# INSTALLED BUILD CHECK
if CONFIG.get_setting('installed') == 'true':
    logging.log("[Build Installed Check] התחיל", level=xbmc.LOGINFO)
    installed_build_check()
else:
    logging.log("[Build Installed Check] לא מופעל", level=xbmc.LOGINFO)
...
if CONFIG.get_setting('keeptrakt') == 'true':
    ...
    logging.log("[Trakt Data] לא מופעל", level=xbmc.LOGINFO)

# SAVE DEBRID
if CONFIG.get_setting('keepdebrid') == 'true':
    logging.log("[Debrid Data] מופעל", level=xbmc.LOGINFO)
    save_debrid()
else:
    logging.log("[Debrid Data] לא מופעל", level=xbmc.LOGINFO)

# SAVE LOGIN
if CONFIG.get_setting('keeplogin') == 'true':
    logging.log("[Login Info] מופעל", level=xbmc.LOGINFO)
    save_login()
else:
    logging.log("[Login Info] לא מופעל", level=xbmc.LOGINFO)

# AUTO CLEAN
if CONFIG.get_setting('autoclean') == 'true':
    logging.log("[Auto Clean Up] מופעל", level=xbmc.LOGINFO)
    auto_clean()
else:
    logging.log('[Auto Clean Up] לא מופעל', level=xbmc.LOGINFO)
